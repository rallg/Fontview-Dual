...
struct _SushiFontWidget {
  ...
  const gchar *otherlang_text_a;
  const gchar *otherlang_text_b;
  gchar *sample_string;
  gchar *font_name;
};
...
build_strings_for_face (SushiFontWidget *self)
{
  ...
  self->otherlang_text_a = otherlang_text_a_stock;
  self->otherlang_text_b = otherlang_text_b_stock;
                                ...
                                gint *min_height)
{
  ...
  if (self->otherlang_text_a != NULL) {
    text_extents (cr, self->otherlang_text_a, &extents);
    pixmap_height += font_extents.ascent + font_extents.descent +
      extents.y_advance + LINE_SPACING;
    pixmap_width = MAX (pixmap_width, extents.width + padding.left + padding.right);
  }

  if (self->otherlang_text_b != NULL) {
    text_extents (cr, self->otherlang_text_b, &extents);
    pixmap_height += font_extents.ascent + font_extents.descent +
      extents.y_advance + LINE_SPACING;
    pixmap_width = MAX (pixmap_width, extents.width + padding.left + padding.right);
  }
                        ...
                        cairo_t *cr)
{
  ...
  if (self->otherlang_text_a != NULL)
    draw_string (self, cr, padding, self->otherlang_text_a, &pos_y);
  if (pos_y > allocated_height)
    goto end;

  if (self->otherlang_text_b != NULL)
    draw_string (self, cr, padding, self->otherlang_text_b, &pos_y);
  if (pos_y > allocated_height)
    goto end;
